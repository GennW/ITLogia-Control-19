...
export class IncomeEdit {
    ...
    private async updateIncomeCategory(title: string): Promise<void> {
        try {
            ...
            const result: RouteParamsType = await CustomHttp.request(config.host + '/categories/income/' + this.routeParams.id, 'PUT', data);
            // Проверяем наличие ошибок в результате запроса
            if (result.error) {
                throw new Error('Ошибка при получении категорий доходов');
            }
    ...
    private initializeUpdateIncomeCategoryListener(): void {
        ...
        const inputValidation: InputValidation = new InputValidation('edit-category-input'); //можно передать в роуты
        ...
        if (saveModifiedCategoryElement) {
            ...
            saveModifiedCategoryElement.addEventListener('click', () => {
                const title: string | undefined = inputValidation.inputElement?.value;
                if (title !== undefined) { // Проверяем, что title не равен undefined
                    this.updateIncomeCategory(title); // Вызываем метод обновления категории с новым заголовком
                } else {
                    console.error('Не удалось получить корректное значение заголовка');
                }
            });
        }
